 ...
 */
public class ChartView extends View implements InterfaceTouch {
...
//        canvas.drawColor(Color.BLACK);
        ...
        Log.i(TAG, "onDraw: yOri:" + yOri);
        Shader mShader = new LinearGradient(100, 0, 100, dpToPx(120), new int[]{Color.BLACK, Color.WHITE}, null, Shader.TileMode.REPEAT);
...
//        canvas.drawPaint(paint);s
        canvas.drawRect(70,0,dpToPx(420),dpToPx(520),paint);

        drawXY(canvas);
//        Log.i(TAG, "height:" + height);
//        canvas.drawText("利润利润", dpToPx(100), height + dpToPx(20), xyTextPaint);
    ...
    private void drawBrokenLine(Canvas canvas) {
        ...
        for (int i = 1; i < xValue.size(); i++) {
            ...
            path.lineTo(x, y);
            path1.lineTo(x, y - dpToPx((int) (15 * Math.random())));
            Log.i(TAG, "drawBrokenLine: 绘制折线 x：" + x + "Y:" + y);
        }
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < yValue.size(); i++) {
            ...
            //绘制横线
            canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2,
                    width + length, yOri - yLength * i + xylinewidth / 2, xyPaint);
            xyTextPaint.setColor(xytextcolor);
            //绘制Y轴文本
            ...
            //绘制右侧Y轴文本
            canvas.drawText(text, 0, text.length(), width - rect.width(),
                    yOri - yLength * i + rect.height() / 2, xyTextPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                startX = event.getX();
                if (mInterfaceTouch != null) {
                    mInterfaceTouch.action_down(this, startX);
                }
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (interval * xValue.size() > width - xOri) {//当期的宽度不足以呈现全部数据
                    ...
                    if (mInterfaceTouch != null) {
                        Log.i(TAG, "onTouchEvent: ACTION_MOVE startX:" + startX + "xInit:" + xInit);
                        mInterfaceTouch.action_move(this, startX, xInit);
                    }
                    invalidate();
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mInterfaceTouch != null) {
                    mInterfaceTouch.action_up(this, event);
                }
                break;
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mInterfaceTouch != null) {
                    mInterfaceTouch.action_cancel(this);
                }
                break;
        }
        return true;
    ...
    public void setSelectIndex(int selectIndex) {
        ...
        //更新
        if (selectIndex < 11) {
            xInit = -interval * (selectIndex - 4) + xOri;
        }
        Log.i(TAG, "xInit:" + xInit + "xOri:" + xOri);
        invalidate();
    }
    ...
    private InterfaceTouch mInterfaceTouch;

    public void setOnTouch(InterfaceTouch interfaceTouch) {
        mInterfaceTouch = interfaceTouch;
    }

    @Override
    public void action_down(Object view, float startXf) {
        Log.i(TAG, "startXf:" + startXf);
        startX = startXf;
    }

    @Override
    public void action_move(Object view, float startXf, float xInitf) {
        startX = startXf;
        xInit = xInitf;

        Log.i(TAG, "startXf:" + startXf + "xInit:" + xInit);
        invalidate();
    }
